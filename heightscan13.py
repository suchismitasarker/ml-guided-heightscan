...
# Detector column ranges (x-pixel) that are always masked out when locating
# the Top-3 peak positions.  These cover edge artefacts and module gaps.
# Format: list of (x_start, x_end) inclusive column ranges.
DETECTOR_DEAD_COLS = [(0, 100), (2400, 2600)]
...
<div class="app-shell">
  ...
  <div class="main-col">
    <header>
      <div>
        <h1>HeightScan — CBF Image Series Line Plot</h1>
...
def find_top_n_peaks(img_arr, n=3, nms_pad=100, dead_cols=None):
    """
    ...
    # Zero out permanently excluded column bands first
    if dead_cols:
...
def compute_peak_stats(z_arr, i_arr):
    ...
    area = float(np.trapz(I, z)) if n > 1 else float(I[0])
...
def fit_gpr_profile(z_arr, i_arr):
    ...
    try:
        ...
        if len(z) > 200:
            idx = np.round(np.linspace(0, len(z) - 1, 200)).astype(int)
...
def run():
    ...
    # ── Pass 1: ROI integration + pixel inspector stats ───────────────────────
    t0 = time.perf_counter()
    z_vals, i_vals, fnames = [], [], []
    px_max_vals, px_x_vals, px_y_vals, px_mean_vals, px_std_vals = [], [], [], [], []
    errors = []
    accum_img = None   # element-wise max across all frames (for peak detection)

    for z in selected:
        try:
            img   = load_cbf(file_map[z])
            ...
            # Accumulate element-wise max (no extra I/O — reuse already-loaded img)
    ...
    if accum_img is not None and h_img > 0 and w_img > 0:
        work = accum_img.copy()
        # Mask permanent detector dead zones
    ...
    # ── Pass 2: integrate all 3 ROIs in a single scan ─────────────────────────
    mr_i_vals  = []   # peak 1
    mr2_i_vals = []   # peak 2
    mr3_i_vals = []   # peak 3
    for z in z_vals:
    ...
    # ML Insights: recommended peak (highest SNR among the three)
    def _snr_val(stats):
        try:    return float(stats['snr'])
        except: return 0.0
    snr_vals  = [_snr_val(mr_stats), _snr_val(mr2_stats), _snr_val(mr3_stats)]
    best_pk   = int(np.argmax(snr_vals))
    _pk_labels = ['🥇 Peak 1', '🥈 Peak 2', '🥉 Peak 3']
    ml_best_peak   = _pk_labels[best_pk]
    ml_best_reason = f"SNR = {snr_vals[best_pk]:.1f} (highest among all 3 peaks)"
